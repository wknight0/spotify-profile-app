...
export default function TopTracks() {
  ...
  var [tracks1, setTracks1] = useState(null);
  var [tracks2, setTracks2] = useState(null);
  var [tracks3, setTracks3] = useState(null);

  useEffect(() => {
    apiClient.get("me/top/tracks?limit=50&time_range=long_term").then((response) => {
      setTracks1(response.data.items);
    });
  }, []);

  useEffect(() => {
    apiClient.get("me/top/tracks?limit=50&time_range=medium_term").then((response) => {
      setTracks2(response.data.items);
    });
  }, []);

  useEffect(() => {
    apiClient.get("me/top/tracks?limit=50&time_range=short_term").then((response) => {
      setTracks3(response.data.items);
      setTimeout(() => {
        dataStage();
      }, 100);
    });
  ...
  function AllTime()
  {
    ...
    document.getElementById("heading").innerHTML = "Top Tracks of All Time";
  }
  ...
  function SixMonths()
  {
    ...
    document.getElementById("button33").style.color = "#909090";
    document.getElementById("button33").style.textDecoration = "none";
    document.getElementById("heading").innerHTML = "Top Tracks from the Last 6 Months";
  }
  ...
  function FourWeeks()
  {
    ...
    document.getElementById("heading").innerHTML = "Top Tracks from the Last 4 Weeks";
  }
  ...
  return (
    <div className="screen-container text-white bg-primary max-w-[1400px] w-[calc(100vw-100px)] min-h-[100vh] p-[80px] m-[auto] xxs:mb-[80px] sm:mb-[25px] lg:p-[80px] xxs:p-[50px]">
        <div className="flex justify-between mb-[35px] md:flex xxs:hidden">
          <h1 id="heading" className="text-2xl font-Black text-left py-[25px]">Top Tracks of All Time</h1><br></br>
        ...
        <div className="md:hidden xxs:block">
          <div className="flex justify-center">
            <h1 id="heading" className="text-2xl font-Black text-center py-[25px]">Top Tracks of All Time</h1><br></br>
          </div>
          ...
          <ul className="block" id="allTime">
            {tracks1?.map((track) => (
                  <li className="mb-[25px] flex items-center">
                    ...
                    <div>
                      <p id="trackName" className="my-auto text-left font-book font-[500]">{track.name}</p>
